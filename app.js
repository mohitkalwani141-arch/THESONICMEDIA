...
function openCaseStudy(id) {
  ...
  if (!document.getElementById('cs-overlay')) {
    ...
    shell.style.cssText = 'display:none;position:fixed;inset:0;z-index:99999;background:#080808;overflow-y:scroll;-webkit-overflow-scrolling:touch;';
...
<div class="cs-footer">
  ...
  overlay.scrollTop = 0;
  /* Destroy Lenis so its wheel preventDefault() listener is fully detached from window.
     lenis.stop() is not enough — the GSAP ticker keeps calling lenis.raf() every frame. */
  if (window.lenis) { window.lenis.destroy(); window.lenis = null; }
  /* Lock body so background page can't scroll behind the fixed overlay */
  document.body.style.overflow = 'hidden';
